...
Tint_array = np.linspace(50.,550.,6)
"""
Tint_array = np.linspace(100.,700.,8)
...
"""
my_therm_obj.main(M_P, CMF, Teqpl, Tint_array, Zenv=Zenv, FeH_flag=False)

# Recommended: save sequence of interior models in case thermal evol eq. solver fails

data = np.zeros((len(my_therm_obj.f_S),4))
...
np.savetxt('thermal_sequence.dat', data,header='f_S s_mean_TE Tint Rtot',comments='',fmt=fmt)
"""

data = pd.read_csv('thermal_sequence.dat', sep='\s+',header=None,skiprows=1)
...
plt.xlim(0.,max(my_therm_obj.age_points))
